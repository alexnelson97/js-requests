...
sayHelloButton.addEventListener("mouseout", resetButtonColor);
// PROBLEM 4
/*
...
const repeatMyParam = () => {
  const param = "my_string_param";
  axios
    .get(`http://localhost:3000/repeat/${param}`)
    .then(function (response) {
      const repeatTextElement = document.getElementById("repeat-text");
      repeatTextElement.textContent = response.data;
...
const makeQueryRequest = () => {
  const queryParam = "?lang=en&region=us";
  axios
    .get(`http://localhost:3000/query-test?query=${queryParam}`)
    .then(function (response) {
      console.log(response.data);
    })
    .catch(function () {
...
document
  ...
  .addEventListener("click", makeQueryRequest);
////////////////
//INTERMEDIATE//
////////////////
